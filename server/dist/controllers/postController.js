...
const getMissingEnvNames = (names) => names.filter((name) => !process.env[name]?.trim());
const hasCloudinaryConfig = () => getMissingEnvNames([
...
const getHuggingFaceToken = () => process.env.HF_TOKEN?.trim() || process.env.HUGGINGFACE_API_KEY?.trim();
...
const assertHuggingFaceConfig = () => {
    if (!getHuggingFaceToken()) {
        throw new Error("HF_TOKEN is missing. Add your Hugging Face token to server/.env.");
    }
};
const toPublicImageError = (error) => {
    ...
    if (normalized.includes("sufficient permissions") ||
    ...
    if (normalized.includes("no image data") ||
...
const uploadImageBufferToCloudinary = async (buffer) => new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream({ folder: "ai-generations", resource_type: "image" }, (error, result) => {
        if (error) {
            reject(error);
            return;
        }
...
const normalizeTopic = (prompt) => prompt
    .trim()
    .replace(/\s+/g, " ")
    .replace(/[.]+$/g, "");
const getHashtags = (prompt) => {
...
const generateDraftContent = (prompt, tone) => {
    ...
    const body = {
        Creative: "Turn the idea into a visual story, keep the message simple, and give your audience one thing worth remembering.",
        Funny: "Show the human side, make the benefit obvious, and give people a reason to stop scrolling without forcing the punchline.",
        Minimalist: "Lead with the value. Remove the noise. Make the action obvious.",
        Excited: "Share the spark, highlight the transformation, and invite your audience into the next move.",
        Professional: "Use the moment to connect the problem, the value, and the next action in a way that feels clear and useful.",
    };
    const closing = {
...
const buildSocialImagePrompt = (imagePrompt, content) => `
...
`;
const imageResultToBuffer = async (image) => {
    if (typeof image === "string") {
        ...
        if (!response.ok) {
            throw new Error(`Failed to download Hugging Face image: ${response.status}`);
        }
...
const generateHuggingFaceImage = async (imagePrompt, content) => {
    ...
    const request = {
        ...
        parameters: {
            ...
            negative_prompt: "low quality, blurry, distorted, watermark, logo, extra text, bad anatomy",
        },
    };
    try {
        ...
        console.warn("Hugging Face URL output unavailable, falling back to blob:", getErrorMessage(error));
    }
