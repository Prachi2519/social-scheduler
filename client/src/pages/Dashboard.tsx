  ...
  return (
    <section className="mx-auto w-full max-w-7xl">
      ...
      <div className="mt-10 grid gap-6 md:grid-cols-3">
        {stats.map((stat) => (
          <div key={stat.label} className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
          ...
          {activities.map((activity) => (
            <div key={`${activity.platform}-${activity.date}`} className="flex items-center gap-5 px-8 py-5">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-2 truncate text-base font-medium text-slate-600">
                  Published post to {activity.platform}
                </p>
              </div>
